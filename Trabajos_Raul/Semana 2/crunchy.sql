...
--%type
DECLARE
    v_id_plan PLANES_SUSCRIPCION.id%TYPE := 1;
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('Plan: ' || v_plan.nombre_plan);
...
BEGIN
    ...
    IF v_estado = 'S' THEN
        DBMS_OUTPUT.PUT_LINE('Usuario activo');
    ELSE
        DBMS_OUTPUT.PUT_LINE('Usuario inactivo');
    END IF;
END;
/
...
--FOR
DECLARE
    v_id_contenido CONTENIDOS.id%TYPE := 1;
BEGIN
    FOR epi IN (
        SELECT num_episodio, nombre_episodio
        FROM EPISODIOS
        WHERE id_contenido = v_id_contenido
        ORDER BY num_episodio
    ) LOOP
        DBMS_OUTPUT.PUT_LINE(
            'Episodio ' || epi.num_episodio || ': ' || epi.nombre_episodio
        );
    END LOOP;
END;
/
--cursor
DECLARE
    CURSOR c_favoritos IS
        SELECT c.titulo_contenido
        FROM FAVORITOS f
        JOIN CONTENIDOS c ON f.id_contenido = c.id
        WHERE f.id_perfil = 1;
...
BEGIN
    OPEN c_favoritos;
    LOOP
        FETCH c_favoritos INTO v_titulo;
        EXIT WHEN c_favoritos%NOTFOUND;

        DBMS_OUTPUT.PUT_LINE('Favorito: ' || v_titulo);
    END LOOP;
    CLOSE c_favoritos;
END;
/
...
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        DBMS_OUTPUT.PUT_LINE('Error en el pago');
END;
/

--colecciones
DECLARE
    TYPE t_generos IS VARRAY(5) OF GENEROS.nombre_genero%TYPE;
    v_generos t_generos := t_generos('Acción', 'Drama', 'Comedia');

BEGIN
    FOR i IN 1 .. v_generos.COUNT LOOP
        DBMS_OUTPUT.PUT_LINE('Género: ' || v_generos(i));
    END LOOP;
END;
/
...
DECLARE
    v_id_usuario    USUARIOS.id%TYPE := 4;
    v_id_perfil     PERFILES.id%TYPE := 4;
    v_id_contenido  CONTENIDOS.id%TYPE := 4;

    v_estado_usuario   USUARIOS.usuario_activo%TYPE;
    v_suscripcion_activa NUMBER;
    v_disponible_region CHAR(1);
    v_intentos NUMBER := 1;

    TYPE t_contenido IS RECORD (
        titulo   CONTENIDOS.titulo_contenido%TYPE,
        estado   CONTENIDOS.estado%TYPE
    );
    v_contenido t_contenido;

    CURSOR c_historial IS
        SELECT c.titulo_contenido
        FROM HISTORIALES h
        INNER JOIN CONTENIDOS c ON h.id_contenido = c.id
        WHERE h.id_perfil = v_id_perfil;
...
BEGIN
    WHILE v_intentos <= 3 LOOP
        ...
        DBMS_OUTPUT.PUT_LINE('Suscripción activa: ' || v_suscripcion_activa);

        IF v_suscripcion_activa = 0 THEN
            DBMS_OUTPUT.PUT_LINE('Intento ' || v_intentos || ': No posee suscripción activa');
            v_intentos := v_intentos + 1;
            CONTINUE;
        END IF;

        SELECT titulo_contenido, estado
        INTO v_contenido
        FROM CONTENIDOS
        WHERE id = v_id_contenido;

        IF v_contenido.estado <> 'en emision'
           AND v_contenido.estado <> 'terminado' THEN
            DBMS_OUTPUT.PUT_LINE('Contenido no disponible');
            EXIT;
        END IF;

        SELECT COUNT(*)
        INTO v_disponible_region
        FROM CONTENIDO_REGIONES cr
        INNER JOIN REGIONES r ON cr.id_region = r.id
        WHERE cr.id_contenido = v_id_contenido
          AND cr.disponible = 'S';
        DBMS_OUTPUT.PUT_LINE('Disponible en región: ' || v_disponible_region);

        IF v_disponible_region = 0 THEN
            DBMS_OUTPUT.PUT_LINE('Contenido no disponible en su región');
            EXIT;
        END IF;
        ...
        DBMS_OUTPUT.PUT_LINE('Acceso permitido al contenido: ' || v_contenido.titulo);
        EXIT;
    END LOOP;

    DBMS_OUTPUT.PUT_LINE('--- Historial del perfil ---');
    FOR r IN c_historial LOOP
        DBMS_OUTPUT.PUT_LINE('Visto: ' || r.titulo_contenido);
    END LOOP;

EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('Usuario, perfil o contenido no existe');
    WHEN OTHERS THEN
        ROLLBACK;
...
SELECT * FROM CONTENIDOS WHERE id = 4;
SELECT * FROM CONTENIDO_REGIONES WHERE id=4;

DESC CONTENIDO_REGIONES;