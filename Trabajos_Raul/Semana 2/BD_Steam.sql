...
BEGIN
    ...
    IF v_saldo >= 20000 THEN
        DBMS_OUTPUT.PUT_LINE('Saldo suficiente, puede comprar el juego :D');
    ELSE
        DBMS_OUTPUT.PUT_LINE('Saldo insuficiente, no puede comprar el juego :c');
    END IF;
END;
/
...
DECLARE
    v_contador NUMBER := 1;
    v_password_hash USUARIOS.password_hash%TYPE;
    v_password_hashed VARCHAR2(255);

BEGIN
    v_password_hashed := '1234';
    WHILE v_contador <= 5 LOOP
        SELECT password_hash INTO v_password_hash FROM USUARIOS WHERE id=1;
        IF v_password_hash <> v_password_hashed THEN
            DBMS_OUTPUT.PUT_LINE('Intento fallido numero: ' || v_contador);
            v_contador := v_contador + 1;
        ELSE 
            DBMS_OUTPUT.PUT_LINE('Inicio de sesión exitoso: ' || v_contador);
        END IF;
    END LOOP;
END;
/
...
DECLARE
    CURSOR c_biblioteca IS
        (SELECT j.nombre FROM BIBLIOTECA_USUARIO b
        JOIN JUEGOS j ON b.id_juego = j.id
        WHERE b.id_usuario = 1)
    ;
BEGIN
    FOR c in c_biblioteca LOOP
        DBMS_OUTPUT.PUT_LINE('Juego en biblioteca: ' || c.nombre);
    END LOOP;
END;
/
--transacciones
...
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        DBMS_OUTPUT.PUT_LINE('Error en la compra');
END;
/
--record
DECLARE
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE(
        'Juego: ' || v_juego.nombre || ' - $' || v_juego.precio
    );
END;
/
